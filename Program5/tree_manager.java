...
public class tree_manager {
    ...
    public void display_math() {
        //if the root is empty there is nothing to display
        if (root == black_nil) {
            System.out.println("No math topics to display");
        } else
            display_math(root);
    }

    //private recursive display function
    private void display_math(tree_node root) {

        //if root isn't null, then:
        if (root != black_nil) {
            //do an inorder display of left, then the data in root, then right:
            display_math(root.go_left());
            root.display_math();
            System.out.println();
            display_math(root.go_right());
        }
    }
    ...
    public void display_chemistry() {
        //if the root is empty there is nothing to display
        if (root == black_nil) {
            System.out.println("No chemistry topics to display");
        } else
            display_chemistry(root);
    }

    //private recursive display function
    private void display_chemistry(tree_node root) {

        //if root isn't null, then:
        if (root != black_nil) {
            //do an inorder display of left, then the data in root, then right:
            display_chemistry(root.go_left());
            root.display_chemistry();
            System.out.println();
            display_chemistry(root.go_right());
        }
    }
    ...
    public void display_CS() {
        //if the root is empty there is nothing to display
        if (root == black_nil) {
            System.out.println("No computer science topics to display");
        } else
            display_CS(root);
    }

    //private recursive display function
    private void display_CS(tree_node root) {

        //if root isn't null, then:
        if (root != black_nil) {
            //do an inorder display of left, then the data in root, then right:
            display_CS(root.go_left());
            root.display_CS();
            System.out.println();
            display_CS(root.go_right());
        }
    }
    ...
    //private retrieve function for the tree:
    ...
    //public non-recursive retrieve function that calls recursive function,
    //this retrieve function calls the retrieve function meant for a subject in
    //the file tre_node with the same name.
    public list_node retrieve_subject (String to_retrieve) {
        if (root == black_nil) {
            return null;
        }
        return retrieve_subject(root, to_retrieve);
    }

    //private retrieve function for the tree:
    private list_node retrieve_subject(tree_node root, String to_retrieve) {

        //as long as we have something to look for:
        if (root != black_nil) {
            //temp will be the node we use to check for the existence of a root
            list_node temp;
            temp = root.retrieve_subject(to_retrieve);

            if (temp != null) {
                return temp;
            } else {
                temp = retrieve_subject(root.go_left(), to_retrieve);
                if (temp != null) {
                    return temp;
                } else {
                    return retrieve_subject(root.go_right(), to_retrieve);
                }
            }
        } else
            return null;
